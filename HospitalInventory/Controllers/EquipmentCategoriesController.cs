...
namespace HospitalInventory.Controllers
{
    public class EquipmentCategoriesController : Controller
    {
        ...
        {
            //CHECK if the equipment category name already exists
            string EquipmentCategoryNameCheck = db.EquipmentCategories.Where(x => x.EquipmentCategoryName.Equals(equipmentCategory.EquipmentCategoryName)).Select(x => x.EquipmentCategoryName).SingleOrDefault();
            if (EquipmentCategoryNameCheck == null)
            {
            ...
            {
                ViewBag.message = String.Format("Creation Failed! Equipment Category with name as {0} already exists", equipmentCategory.EquipmentCategoryName);
                return View();
            }
        }
